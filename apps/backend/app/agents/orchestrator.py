...
def _template_speech(policy: Any, state: GameState, seat_id: int) -> str:
    """无 LLM 时的发言模板：带**人设口吻**，依据心证点名最可疑者。"""
    beliefs = getattr(policy, "beliefs", {}) or {}
    persona = getattr(policy, "persona", None)
    tic = (persona.tics[(seat_id + state.round) % len(persona.tics)] + "，") if persona and persona.tics else ""
    opener = tic or _OPENERS[(seat_id + state.round) % len(_OPENERS)]
    suspects = sorted(
        ((b.trust, s) for s, b in beliefs.items() if getattr(b, "suspected_faction", None) == "werewolf"),
    )
    if suspects:
        return f"{opener}我觉得 #{suspects[0][1]} 很有问题，建议重点盯他。"
    distrust = sorted(((getattr(b, "trust", 0), s) for s, b in beliefs.items()))
    if distrust and distrust[0][0] < 0:
        return f"{opener}我对 #{distrust[0][1]} 有点怀疑，先听听他怎么说。"
    return f"{opener}目前信息不多，我先保持观察，不急着站队。"
...
def step_preemptive(
    ...
    intents = []
    for s in ai:
        act = policies[s.seat_id].decide(engine, gs, s)
        if on_think is not None:
            on_think(s.seat_id, getattr(policies[s.seat_id], "last_thought", {}))
        intents.append((s.seat_id, act))
    # 2) 顺序仲裁：按座位序应用
...
def step_queued(
    ...
    on_event: Optional[Callable] = None, human_seats: tuple = (), on_think=None,
) -> bool:
...
def drive(
    ...
    while not gs.finished and steps < max_steps:
        ...
        if phase_mode(gs) == QUEUED:
            if step_queued(engine, gs, policies, on_event, human_seats, on_think=on_think):
                return
        else:
            if step_preemptive(engine, gs, policies, human_seats, on_think=on_think):
                return
